...
map <string, int> command_handle::savecolumn(const vector <string>& separated_line)
{
	map <string, int> column;

	for (int i = 0; i < separated_line.size(); i++)
		column[separated_line[i]] = i;
...
void command_handle::save_songs_in_program(char songs_list[])
{
	...
	map <string, int> column = savecolumn(separated_line);
...
void command_handle::run(char songs_list[])
{
