...
void recommended_songs::print_recommended_songs(const vector <pair <song*, double>>& songs_confidence)
{
	for (int i = 0; ((i < count) && (i != songs_confidence.size())); i++)
	{
		cout << songs_confidence[i].first->get_id() << " " << setprecision(PRECISION_NUMBER) << fixed
			<< songs_confidence[i].second << "% ";
		songs_confidence[i].first->write_some_info();
	}
}

void recommended_songs::recommeded_songs_function()
{
	vector <vector <bool>> users_and_songs_matrix;
	vector <vector <double>> users_similarity_matrix;
	...
	print_recommended_songs(songs_confidence);
}